...
HEREDOC
	...
	readWrite | while read rw er; do
		...
		echo "// ${rw}Bool ${rw}s a boolean.";
		echo -n "func (e *Mem${e}Endian) ${rw}Bool(";
		if [ "${rw}" = "Write" ]; then
			echo "b bool) {";
...
HEREDOC
		...
		numberTypes | while read t i ti; do
			...
			echo -n "func (e *Mem${e}Endian) ${rw}${t}${i}(";
		...
		bytesStrings | while read t type empty tt; do
			...
			echo "}";
				...
				if [ "$t" = "Bytes" ]; then
					...
					echo "func (e *Mem${e}Endian) ${rw}(p []byte) (int, error) {";
			...
			for size in "X" 8 16 24 32 40 48 56 64; do
				...
				echo;
				echo "// ${rw}${t}${size} ${rw}s the length of the ${t}, using ${rw}Uint${size} and then ${rw}s the bytes.";
				echo -n "func (e *Mem${e}Endian) ${rw}${t}${size}(";
		...
		echo -n "func (e *Mem${e}Endian) ${rw}String0(";
	...
	) > "$(echo "mem${e}endian" | tr A-Z a-z).go";
done;
