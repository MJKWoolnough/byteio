...
func TestLittleEndianVarUint(t *testing.T) {
	...
	for n, test := range littlevarintTests {
		...
		if num, _, _ := (&LittleEndianReader{Reader: buf}).ReadUintX(); num != test.Input {
...
func TestStickyLittleEndianVarUint(t *testing.T) {
	...
	for n, test := range littlevarintTests {
		...
		if num := (&StickyLittleEndianReader{Reader: buf}).ReadUintX(); num != test.Input {
			t.Errorf("test %d.2: expecting %d, got %d", n+1, test.Input, num)
		}
	}
}
...
func TestBigEndianVarUint(t *testing.T) {
	...
	for n, test := range bigvarintTests {
		...
		if num, _, _ := (&BigEndianReader{Reader: buf}).ReadUintX(); num != test.Input {
			t.Errorf("test %d.2: expecting %d, got %d", n+1, test.Input, num)
		}
	}
}

func TestStickyBigEndianVarUint(t *testing.T) {
	buf := bytes.NewBuffer(make([]byte, 0, 9))

	for n, test := range bigvarintTests {
		buf.Reset()
		(&StickyBigEndianWriter{Writer: buf}).WriteUintX(test.Input)
